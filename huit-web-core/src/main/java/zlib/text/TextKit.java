...
public final class TextKit
{
	...
	/** 大写英文字符集 */
	public static final char[] UPPER_ENGLISH={FIRST_UPPER_ENGLISH,
		LAST_UPPER_ENGLISH};
	/** 小写英文字符集 */
	public static final char[] LOWER_ENGLISH={FIRST_LOWER_ENGLISH,
		LAST_LOWER_ENGLISH};
	/** 英文字符集 */
	public static final char[] ENGLISH={FIRST_UPPER_ENGLISH,
		LAST_UPPER_ENGLISH,FIRST_LOWER_ENGLISH,LAST_LOWER_ENGLISH};
	/** ASCII字符集 */
	 ...
	 * {'零','一','二','三','四','五','六','七','八','九'};
	 */
	public static final char[] DIGITS={'\u96F6','\u4E00','\u4E8C','\u4E09',
		'\u56DB','\u4E94','\u516D','\u4E03','\u516B','\u4E5D'};
	/**
	 * 汉字的量词数组，可以重设该数组中的汉字，
	 * {'','十','百','千','万','亿'};
	 */
	public static final char[] DIGIT_BITS={' ','\u5341','\u767E','\u5343',
		'\u4E07','\u4EBF'};
	/**
	 * 汉字的大写数词数组，可以重设该数组中的汉字，
	 * {'零','壹','贰','叁','肆','伍','陆','柒','捌','玖'};
	 */
	public static final char[] UPPER_DIGITS={'\u96F6','\u58F9','\u8D30',
		'\u53C1','\u8086','\u4F0D','\u9646','\u67D2','\u634C','\u7396'};
	/**
	 * 汉字的大写量词数组，可以重设该数组中的汉字，
	 * {'','拾','佰','扦','萬','亿'};
	 */
	public static final char[] UPPER_DIGIT_BITS={' ','\u62FE','\u4F70',
		'\u6266','\u842C','\u4EBF'};
	...
	public static long parseLong(String str)
	{
		...
		if(str.charAt(0)=='#') return Long.parseLong(str.substring(1),16);
	...
	/**
	 * 字符串替换方法，
	 * 把str字符串中的从start到start+count的子字符串替换为swap
	 */
	...
	/**
	 * 字符串替换方法，使用java默认的字符串查找方法，
	 * str为字符串，target为目标字符串，
	 * swap为替换字符串，caseless为忽略大小写，
	 * cb为临时使用的字符串缓存
	 */
	public static String replace(String str,String target,String swap,
		boolean caseless,CharBuffer cb)
	{
		String str1=str;
	...
	/**
	 * 替换全部的字符串替换方法，使用java默认的字符串查找方法，
	 * str为字符串，target为目标字符串，swap为替换字符串
	 */
	...
	{
		cb.append(obj.getClass().getName()).append('@').append(
			obj.hashCode());
	}
	/** 将指定的布尔数数组转换成字符串 */
	...
	/** 将指定的布尔数数组转换成字符串 */
	public static String toString(boolean[] array,String separator)
	{
	...
	/** 将指定的布尔数数组转换成字符串 */
	public static void toString(boolean[] array,String separator,
		CharBuffer cb)
	{
		int n=array.length-1;
		for(int i=0;i<n;i++)
			cb.append((array[i])?'1':'0').append(separator);
		if(n>=0) cb.append(array[n]);
	}
	/** 将指定的字节数组转换成字符串 */
	...
	/** 将指定的字节数数组转换成字符串 */
	public static void toString(byte[] paramArrayOfByte, int paramInt1, int paramInt2, String paramString, CharBuffer paramCharBuffer)
	{
	    int i = paramInt1 + paramInt2 - 1;
	    for (int j = paramInt1; j < i; ++j)
	      paramCharBuffer.append(paramArrayOfByte[j]).append(paramString);
	    if (i < 0) return; paramCharBuffer.append(paramArrayOfByte[i]);
	}
	/** 将指定的短整数数组转换成字符串 */
	...
	{
		if(subString==null||subString.length()==0) return -1;
		...
		while(true)
		{
			...
			if(k>0)
			{
				...
				if(k<0) continue;
				if(str.indexOf(separator,k)!=k) continue;
			}
			k=i;
			if(k==str.length()) return j;
			if(k+separator.length()>str.length()) return -1;
			if(str.indexOf(separator,k)!=k) continue;
			return j;
		}
	}
	/** 二次劈分，获得指定键的值 */
	public static String getSubValue(String str,String subKey,
		String separator,String subSeparator)
	{
		if(subKey==null||subKey.length()==0) return null;
		if(subKey.length()+subSeparator.length()>str.length()) return null;
		int i=0,j=0,k=0;
		while(true)
		{
			...
			k=i;
			if(k+subSeparator.length()>str.length()) return null;
			if(k+subSeparator.length()==str.length()) return EMPTY_STRING;
			if(str.indexOf(subSeparator,k)!=k) continue;
			i=k+subSeparator.length();
			j=str.indexOf(separator,i);
			if(j<0) return str.substring(i);
	 ...
	 * 
	 * @param
	 * charRangeSet为允许通过的字符范围集，两个字符为一组范围，
	...
	/**
	 * 正则表达式的$1功能*，
	 * 得到第一个begin和end间的字符串（不含），从指定的偏移量开始查找
	 */
	public static String betweenString(String str,String begin,String end,
		int offset)
	{
		...
		if(begin.length()+end.length()+offset>str.length()) return null;
	 ...
	 * 
	 * @param number 表示要转换的数字，必须为正数
	 * @param cb 表示转换后的汉字数字要写入的CharBuffer
	 * @param digits 表示使用的汉字数字数组
	 * @param bits 表示使用的汉字单位数组
	 * @param fillZero 表示前面是否补零
	 */
	public static void toChineseNumber(int number,CharBuffer cb,
		char[] digits,char[] bits,boolean fillZero)
	{
		if(number<0)
			throw new IllegalArgumentException(toString
				+" toChineseNumber, invalid number, number="+number);
		if(number>=100000000)
		{
		...
		for(int i=3,n=1000;i>=0;i--,n/=10)
		{
			...
			if(t!=0)
			{
				...
				cb.append(digits[t]);
				if(i>0) cb.append(bits[i]);
				fillZero=true;
			}
			else
			{
				if(fillZero) zero=true;
			}
			number%=n;
			if(number==0) return;
		}
